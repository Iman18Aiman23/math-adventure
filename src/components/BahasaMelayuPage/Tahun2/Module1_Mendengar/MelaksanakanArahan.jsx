...
const TOPIC_ID = '2-1-1b-arahan';
const ACCENT = '#E8821A';

const CATEGORIES = [
  {
    title: 'Dengar Arahan dengan Teliti',
    desc: 'Arahan ada beberapa langkah. Dengar semuanya dahulu.',
    example: '"Ambil pensel, lukis bulatan, warnakan kuning." → 3 langkah',
    usage: 'Ingat langkah PERTAMA hingga TERAKHIR.',
  },
  {
    title: 'Ikut Susunan',
    desc: 'Buat mengikut urutan yang betul, satu demi satu.',
    example: 'Langkah 1 → Langkah 2 → Langkah 3',
    usage: 'Tanya: "Apa dulu? Apa kemudian?"',
  },
];

function MelaksanakanArahanLearnPage({ onBack, onStartQuiz, topicTitle, language }) {
      ...
      <div className="kba-learn-root">
        ...
        <div className="kba-body">
          <div className="kba-heading">
            <h1>{language === 'bm' ? 'Melaksanakan Arahan 🎯' : 'Follow Instructions 🎯'}</h1>
            <p>{language === 'bm' ? 'Ketuk untuk belajar cara ikut arahan' : 'Tap to learn how to follow instructions'}</p>
          </div>
...
export default function MelaksanakanArahan({ onBack, language = 'bm', topicComplete, onNextTopic }) {
  ...
  const topicTitle = language === 'bm' ? 'Melaksanakan Arahan' : 'Follow Instructions';

  if (page === 'learn') {
    return (
      <MelaksanakanArahanLearnPage
        onBack={() => onBack?.()}
