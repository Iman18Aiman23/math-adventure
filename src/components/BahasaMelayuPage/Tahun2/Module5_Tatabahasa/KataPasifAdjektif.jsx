...
const TOPIC_ID = '2-5-1b-kerja-pasif-adjektif';
const ACCENT = '#159E96';
...
function KataPasifAdjektifLearnPage({ onBack, onStartQuiz, topicTitle, language }) {
      ...
      <div className="kba-learn-root">
        ...
        <div className="kba-body">
          <div className="kba-heading">
            <h1>{language === 'bm' ? 'Kata Kerja Pasif & Kata Adjektif 🔄🎨' : 'Passive Verbs & Adjectives 🔄🎨'}</h1>
...
export default function KataPasifAdjektif({ onBack, language = 'bm', topicComplete, onNextTopic }) {
  ...
  const topicTitle = language === 'bm' ? 'Kata Kerja Pasif & Kata Adjektif' : 'Passive Verbs & Adjectives';

  if (page === 'learn') {
    return (
      <KataPasifAdjektifLearnPage
        onBack={() => onBack?.()}
