...
- **2026-06-14 — R13 / 1.2 — Bercerita & Berbincang (handoff agen).** Fail diubah: (1) `BerceritaBincang.jsx` — komponen dua-tier baru (Tier A: 3 cerita × 3 ayat = 9 item baca-kuat dengan STT lenient match + fallback lapor-diri "Saya dah baca ✅"; Tier B: 6 soalan MCQ berbincang berpandu; result screen dengan gate 70% + `completeTopic`); guna scaffolding fasa daripada `BerceritaBerbincang.jsx` (phase-machine, mic UX, `SpeechManager`); (2) `App.jsx` — tambah lazy import `BerceritaBincang` + route Pattern-1 (tiada `ProgressWrapper`, hantar `topicComplete`/`onNextTopic`); import `BertuturBertatasusila` dan route AgeGroup-7 standalone ditinggalkan tidak berubah; (3) `T2_COVERAGE_REPORT.md` — set R13 ke 🔍 Pending verification + nota serah-tangan ini. Diuji: `npm run build` hijau. Fallback mic-denied diuji secara kod — cawangan `permanentFallback` (cek `getUnsupportedReason()`) dan `tempFallback` (ralat `perm`/`net`) kedua-dua render butang "Saya dah baca ✅". Status → 🔍 Pending verification (menunggu semakan bebas).
