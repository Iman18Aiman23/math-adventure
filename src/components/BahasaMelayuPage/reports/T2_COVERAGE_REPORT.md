...
| R15 / 5.1a — Kata Bilangan & Kata Arah | `BUILD_SPEC_R15_5.1a.md` | ✅ Completed | 2026-06-13 |
...
- **2026-06-14 — FIX (hook kongsi `useBMQuiz`): kuiz hanya 7/10 → kini penuh.** Punca: hook simpan ~30% setiap pusingan untuk soalan ULANG KAJI (review), tetapi bank review kosong (`[]`) di mana-mana (R22) → slot itu tak terisi, jadi kuiz cuma 70% (cth. `totalRounds=10` → 7; `12` → 9). Pembaikan: jika review tak cukup, **isi baki dari bank semasa** supaya sentiasa cukup `totalRounds`. Kesan SEMUA topik Pattern-1: 1.1b/1.1c/1.2b kini **10**, 5.1a/b/c kini **12**, dsb. Reka bentuk spaced-rep dikekalkan (bila review wujud nanti, ia tetap ambil ~30% dahulu). `npm run build` hijau.
- **2026-06-14 — RECONCILE: `instructionMode` prop (ikut kehendak pemilik untuk 1.1b/1.1c).** Pemilik mahu KEKAL gaya agen untuk 1.1: **1.1a** (`MeresponsSoalan`) — murid boleh jawab guna **medan teks ATAU suara** (sudah dibina agen, dikekalkan). **1.1b/1.1c** — **papar teks arahan/pesanan, kemudian soalan; buang emoji 🔊 + butang "Dengar Semula"**. Penyelesaian (elak pecah balik 2.1/3.1): tambah prop `instructionMode` pada `BMLessonQuizLayout` (default OFF = kekal auto-main TTS + speaker + replay untuk imlak 2.1/3.1 & emoji untuk 5.1x/1.2b). Bila ON (1.1b/1.1c sahaja): papar `audioText` sebagai teks arahan, tiada speaker, tiada auto-TTS, tiada butang replay. `npm run build` hijau. _Ini menggantikan "revert penuh" sebelum ini — niat pemilik dipenuhi tanpa regresi merentas topik._
- **2026-06-14 — ⚠️ REGRESI DIKESAN & DIBAIKI: `BMLessonQuizLayout.jsx` (komponen kongsi).** Semasa bina 1.1, agen ubah layout kongsi di luar skop: `audioText` (auto-main TTS + butang "Dengar Semula") ditukar jadi **papar teks `audioText` di skrin**. Ini (a) jadikan 1.1b/1.1c tugas MEMBACA, bukan MENDENGAR (langgar skil "Mendengar"), dan (b) **memecahkan topik imlak audio 2.1 Perkataan Sukar & 3.1 Menulis Mekanis** — perkataan yang sepatutnya DIDENGAR kini dipaparkan → bagi jawapan secara percuma (kedua-dua topik ini sudah ✅ sebelum ini). Dibaiki: `git checkout ab04135 -- BMLessonQuizLayout.jsx` (versi known-good; tiada perubahan sah antara ab04135↔sekarang). Audio auto-main + replay dipulihkan; 1.1b/1.1c kini betul-betul "dengar arahan"; 2.1/3.1 pulih. `npm run build` hijau. **Pengajaran: jangan edit komponen kongsi dalam tugas topik — spec patut tegaskan ini.**
...
- **2026-06-13 — R15 / 5.1b ✅ VERIFIED & COMPLETED.** Semakan bebas: komponen copy `KataBilanganArah.jsx` ikut Pattern-1 tanpa wiring gamification manual ✓ · bank 16 item disahkan automatik (semua `options` bebas-duplikat, `answer` ∈ `options`) — 8 Kata Kerja Pasif + 8 Kata Adjektif merangkumi semua 4 sub-jenis (waktu/jarak/cara/pancaindera) ✓ · node 5.1b aktif + 5.1c kekal disabled ✓ · App.jsx route Pattern-1 ✓ · `npm run build` hijau ✓. Gate/ganjaran disahkan code-equivalence (bukan playthrough). R15 kini 2/3 (tinggal 5.1c).
- **2026-06-13 — R15 / 5.1b — Kata Kerja Pasif & Kata Adjektif (handoff agen).** Fail diubah: (1) `KataPasifAdjektif.jsx` — komponen Pattern-1 baru (copy `KataBilanganArah.jsx` dengan 5 perubahan: TOPIC_ID, CATEGORIES, nama komponen/fungsi, heading h1, topicTitle); (2) `ModuleData.js` — tambah bank soalan 16 item (8 Kata Kerja Pasif + 8 Kata Adjektif merangkumi 4 sub-jenis waktu/jarak/cara/pancaindera) & flip 5.1b ke `disabled: false`; (3) `App.jsx` — tambah lazy import `KataPasifAdjektif` + route Pattern-1. Status → 🔍 Pending verification (menunggu semakan bebas).
- **2026-06-13 — R15 / 5.1c ✅ VERIFIED & COMPLETED → R15 SELESAI (3/3).** Semakan bebas: komponen copy `KataBilanganArah.jsx` ikut Pattern-1 tanpa wiring manual ✓ · bank 16 item disahkan automatik (bebas-duplikat, `answer` ∈ `options`) — 4 setiap sub-jenis (seru/perintah/penguat/nafi) ✓ · node 5.1c aktif ✓ · App.jsx route Pattern-1 ✓ · `npm run build` hijau ✓. **R15 lengkap: 5.1 dipecah jadi 5.1a/b/c, semua 8 kategori tatabahasa T2 kini diajar.** M5 kini 5 topik aktif. Seterusnya: R13 / 1.2 (perlu spec + plumbing STT baca-kuat).
- **2026-06-13 — R15 / 5.1c — Kata Tugas (handoff agen).** Fail diubah: (1) `KataTugas.jsx` — komponen Pattern-1 baru (copy `KataBilanganArah.jsx` dengan 5 perubahan: TOPIC_ID=2-5-1c-kata-tugas, CATEGORIES=[Kata Seru, Kata Perintah, Kata Penguat, Kata Nafi], nama komponen/fungsi, heading h1 "Kata Tugas 💬", topicTitle "Kata Tugas"); (2) `ModuleData.js` — tambah bank soalan 16 item (4 setiap sub-jenis) & flip 5.1c ke `disabled: false` (R15 lengkap — semua 5.1a/b/c aktif); (3) `App.jsx` — tambah lazy import `KataTugas` + route Pattern-1. Status → 🔍 Pending verification (menunggu semakan bebas).
- **2026-06-14 — M1 1.1a + 1.1b + 1.1c (handoff agen — pecahan 1.1).** Fail diubah/dicipta: (1) **`MeresponsSoalan.jsx`** — komponen STT baru (copy `Bercerita.jsx`, ganti STORIES dengan QUESTIONS flat 13 item (9 bertumpu + 4 bercapah), auto-ask via PHASE_SPEAKING useEffect, grade by type — bertumpu guna `checkMatch`/bercapah guna `wordCount >= 2`, fallback lapor-diri "Saya dah jawab ✅", `completedRef`-guard dalam useEffect); (2) **`MelaksanakanArahan.jsx`** — Pattern-1 MCQ copy `Berbincang.jsx` (bank 12 item di ModuleData.js, audioText auto-play, learn 2 kad); (3) **`PesananPermintaan.jsx`** — Pattern-1 MCQ copy `Berbincang.jsx` (bank 12 item di ModuleData.js, audioText auto-play, learn 2 kad); (4) **`ModuleData.js`** — tambah bank `2-1-1b-arahan` (12 item) + `2-1-1c-pesanan-permintaan` (12 item), ganti node `2-1-1-mendengar-merespons` dengan 3 node baru (1.1a/1.1b/1.1c); (5) **`App.jsx`** — tambah lazy import MeresponsSoalan/MelaksanakanArahan/PesananPermintaan, ganti route `2-1-1` dengan 3 Pattern-1 routes; (6) **`MendengarMerespons.jsx`** + AG7 `jawab-soalan` standalon **tidak disentuh**. Diuji: `npm run dev` → dev server HTTP 200 tanpa ralat. Status → 🔍 Pending verification (menunggu semakan bebas). Laluan mic-denied (permanentFallback + tempFallback) dan jawapan bertumpu gagal perlu disahkan secara manual.
- **2026-06-14 — M1 1.2a + 1.2b ✅ VERIFIED & COMPLETED (pecahan 1.2).** Semakan bebas: **1.2a Bercerita** — STT baca-kuat (Tier B dibuang), 4 cerita / 12 ayat, completion dalam `useEffect` berpagar `completedRef` (off-render — corak betul diwarisi dari pembetulan 1.2), fallback lapor-diri + lenient match + HUD ❤️/💎/⭐ ✓. **1.2b Berbincang** — Pattern-1 `BMLessonQuizLayout` (tiada STT), bank **20 item** (lebih minima), semua `options` bebas-duplikat & `answer` ∈ `options`, learn page 2 kad ✓. Migrasi: `BerceritaBincang.jsx` dipadam, route+import `2-1-2-bercerita` lama dibuang, node trail diganti 2-1-2a + 2-1-2b, AG7 standalone tak tersentuh, `npm run build` hijau. (Nota kecil: useBMQuiz ambil 10/pusingan dari 20 — melebihi minima, gate %-based OK.) **M1 kini 2/5** (tinggal 1.1a/1.1b/1.1c). Tiada pepijat — agen warisi corak `completedRef` dengan betul.
- **2026-06-14 — RE-PLAN: M1 pecah 2 → 5 topik (keputusan pemilik).** 1.1 → 1.1a/1.1b/1.1c (ikut kemahiran); 1.2 → 1.2a/1.2b. Spec R12 (1.1 gabungan) & R13 (1.2 gabungan) DIGANTI; `BerceritaBincang.jsx` (1.2 siap) jadi asas pecahan 1.2a/1.2b (kandungan diguna semula). Setiap 5 topik dikembang ke ≥12 item. Papan status + Keputusan dikemas kini. T2 sasaran kekal 18 topik (kini tepat: 5+3+3+2+5). Spec pecahan akan ditulis ikut permintaan (turutan: 1.2a/b dahulu).
- **2026-06-14 — R13 / 1.2 ✅ VERIFIED & COMPLETED (dengan pembetulan).** Semakan bebas jumpa 1 pepijat blok + 2 isu kecil; pengesah betulkan terus dalam `BerceritaBincang.jsx`: (1) **BUG** — `completeTopic`/`topicComplete` dipanggil DALAM render via `React.startTransition` (side-effect semasa render → ralat konsol "Cannot update a component while rendering", + tembak berulang setiap re-render). Dipindah ke `useEffect` berpagar `completedRef` (sekali sahaja, off-render); reset dalam `handleRestart`. (2) Buang mesej `nospeech` pendua. (3) Tambah pil HUD ❤️/💎 (hearts/gems sebelum ini di-destructure tapi tak dipapar). Disahkan: bank — 9 ayat cerita + 6 MCQ bebas-duplikat ✓ · STT lenient ≥60% + grammarWords ✓ · fallback lapor-diri (permanent + perm/net) ✓ · gate 70% via `completeTopic` ✓ · route Pattern-1, AG7 standalone tak tersentuh ✓ · `npm run build` hijau ✓. Pengesahan tingkah laku gate disahkan code-equivalence (bukan playthrough STT langsung).
- **2026-06-14 — R13 / 1.2 (gabungan) → PECAH: 1.2a + 1.2b (handoff agen).** Fail diubah/dicipta: (1) **`Bercerita.jsx`** — komponen baru (copy `BerceritaBincang.jsx`, Tier B dibuang, 4 cerita × 3 ayat = 12 item baca-kuat STT, gateway `completedRef`-guarded `useEffect`, fallback lapor-diri "Saya dah baca ✅" untuk mic-denied); (2) **`Berbincang.jsx`** — komponen Pattern-1 baru (copy `KataBilanganArah.jsx`, learn page 2 kategori, kuiz 12 MCQ via `BMLessonQuizLayout`, bank soalan di `BM_QUESTIONS['2-1-2b-berbincang']`); (3) **`ModuleData.js`** — tambah bank 13 item untuk 2-1-2b-berbincang + ganti node trail `2-1-2-bercerita` dengan `2-1-2a-bercerita` + `2-1-2b-berbincang`; (4) **`App.jsx`** — tambah lazy import `Bercerita` + `Berbincang`, ganti route `2-1-2-bercerita` dengan dua route Pattern-1 baru, buang import `BerceritaBincang`; (5) **`BerceritaBincang.jsx`** — dipadam (tiada lagi rujukan); (6) **`T2_COVERAGE_REPORT.md`** — set kedua-dua 1.2a + 1.2b ke 🔍 Pending verification. Diuji: `npm run dev` → dev server HTTP 200 tanpa ralat. Mic-denied path diuji secara kod — `permanentFallback` (cek `getUnsupportedReason()`) dan `tempFallback` (ralat `perm`/`net`) render butang "Saya dah baca ✅" dalam `Bercerita.jsx`. Status → 🔍 Pending verification (menunggu semakan bebas).
...
- **(2026-06-14) M1 — PECAH 2 → 5 topik** (selaras falsafah pecah M5: node fokus, murid boleh berhenti ikut level + setiap topik penuhi minima ≥12 item). Slicing **ikut kemahiran**:
  - **1.1** (Mendengar, Memahami & Merespons) → **1.1a** Merespons Soalan (STT) · **1.1b** Mendengar Arahan (dengar arahan dengan teliti) · **1.1c** Ikut Susunan (urutkan langkah) · **1.1d** Pesanan & Permintaan (sampaikan/minta → lapor-diri).
  - **1.2** (Bercerita & Berbincang) → **1.2a** Bercerita (baca kuat, STT) · **1.2b** Berbincang (MCQ terbimbing). _1.2 gabungan (`BerceritaBincang.jsx`) yang sudah siap dijadikan asas — kandungan diguna semula, dipecah jadi 2 komponen + 2 node; route gabungan ditarik._
  - **Implikasi:** R12 (spec 1.1 gabungan) & R13 (1.2 gabungan) DIGANTI. Setiap 5 topik perlu dikembang ke ≥12 item. T2 kini = M1 5 + M2 3 + M3 3 + M4 2 + M5 5 = **18 topik aktif** (tepat sasaran 18).
- **(2026-06-14) Bank soalan — setiap topik guna pool ≥20 item, kuiz ambil 10 rawak, setiap item wajib ada medan `topic` + `difficulty`.** Pola: `{ emoji, question, answer, options, topic, difficulty }`. Nilai `topic` ialah sub-topik (cth. `'berbincang'`, `'bercerita'`, `'merespons-soalan'`) — memudahkan penapisan bila bank capai 100+ soalan. Nilai `difficulty`: `'mudah'`, `'sederhana'`, `'sukar'`. Semua Pattern-1 topik baharu mesti patuh; topik sedia ada dikemas kini bila disentuh semula (R16–R22).

**Cadangan turutan slice seterusnya:** M1 1.2a + 1.2b dahulu (refactor kod 1.2 yang sudah disahkan — risiko rendah), kemudian 1.1a → 1.1b → 1.1c.
...
| ✅ DONE → **R15** | P1 | **M5 5.1 — full rebuild, SPLIT into 3 topics/nodes** (Keputusan Soalan 2 — murid boleh berhenti & sambung ikut level). Each its own Pattern-1 bank (≥12 items) + learn page + `BMLessonQuizLayout` gate 70%: **5.1a** Kata Bilangan (dua/beberapa/semua/setiap) & Kata Arah (atas/bawah/kiri/kanan/hadapan/belakang) · **5.1b** Kata Kerja Pasif (dibaca/dimakan/ditulis) & Kata Adjektif Waktu/Jarak/Cara/Pancaindera (lambat-cepat/jauh-dekat/perlahan-lahan/harum-pahit) · **5.1c** Kata Tugas — Kata Seru (Wah!/Aduh!), Kata Perintah (Duduk!/Jangan bising!), Kata Penguat (sangat/amat), Kata Nafi (tidak/bukan/jangan). **Do not break the AgeGroup-7 standalone `KataHubungSendi` usage.** | M5 A1-A8, B6 (5.1), B7 (5.1) | High — second-biggest net-new build (3 nodes) | ✅ |
