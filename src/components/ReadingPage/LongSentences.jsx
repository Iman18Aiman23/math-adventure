...
const LEVEL_DATA = readingData.filter(item => item.level === 4);

export default function LongSentences({ onBack, onComplete = onBack, language, title = language === 'bm' ? 'Ayat Mudah' : 'Simple Sentences' }) {
  ...
  const displayText = script === 'ENG' ? currentItem?.eng : script === 'JAWI' ? currentItem?.jawi : currentItem?.rumi;
  const translationText = script === 'ENG' ? currentItem?.rumi : currentItem?.eng;
  ...
  return (
    <div className="kv-learning">
      ...
      <main className="kv-learning-body">
        <article className="kv-flashcard" aria-label={language === 'bm' ? 'Ilustrasi ayat' : 'Sentence illustration'}>
          <div className="kv-flashcard-picture" aria-hidden="true">{currentItem?.emoji}</div>
          <div className="kv-flashcard-word" dir={script === 'JAWI' ? 'rtl' : 'ltr'} lang={script === 'ENG' ? 'en' : script === 'JAWI' ? 'ms-Arab' : 'ms'}>{displayText}</div>
          <div className="kv-flashcard-translation" lang={script === 'ENG' ? 'ms' : 'en'}>{translationText}</div>
