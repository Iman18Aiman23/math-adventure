...
// KSSR BM Tahun 1 — Obj 3 (bertutur menggunakan bahasa yang bertatasusila).
...
// ── Scenario bank ───────────────────────────────────────────────────────────────
// phrase = model answer shown/spoken; accept = normalised phrases STT may hear.
const SCENARIOS = [
...
// Bias recognition toward the words in the accepted phrases.
...
export default function BertuturBertatasusila({ onBack, language = 'bm' }) {
  ...
  const handleWrong = () => {
    ...
    if (over) {
      // model the polite phrase before moving on
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
          <div>
            <h1 style={{ color: C.primary, fontSize: '1.4rem', fontWeight: 900, marginBottom: '0.1rem' }}>
              🙇 {language === 'bm' ? 'Bertutur Bertatasusila' : 'Polite Speaking'}
            </h1>
            <p style={{ color: '#888', fontSize: '0.82rem' }}>
              {language === 'bm' ? 'Apa yang patut kamu katakan?' : 'What should you say?'}
            </p>
          </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0.5rem 1rem', maxWidth: '600px', width: '100%', alignSelf: 'center', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', justifyContent: 'center' }}>
        ...
        <div style={{ background: cardBg, border: `3px solid ${cardBorder}`, borderRadius: '24px', padding: '1.75rem 1.5rem', textAlign: 'center', width: '100%', transition: 'background 0.3s, border-color 0.3s', boxShadow: '0 4px 16px rgba(0,0,0,0.06)' }}>
          <p style={{ fontSize: '0.8rem', fontWeight: 700, color: '#AAA', marginBottom: '0.75rem', letterSpacing: '0.5px' }}>
            {language === 'bm' ? 'SITUASI' : 'SITUATION'}
          </p>
          {item && (
            <>
              <div style={{ fontSize: '4rem', lineHeight: 1, marginBottom: '0.75rem' }}>{item.emoji}</div>
              <div style={{ fontSize: '1.4rem', fontWeight: 800, lineHeight: 1.35, color: isCorrect ? C.correctDark : isWrong ? C.wrongDark : '#333', transition: 'color 0.3s' }}>
