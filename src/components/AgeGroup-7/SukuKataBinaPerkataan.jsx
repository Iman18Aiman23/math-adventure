...
export default function SukuKataBinaPerkataan({ onBack, language = 'bm' }) {
  ...
  return (
    <div style={{ height: '100dvh', overflow: 'hidden', display: 'flex', flexDirection: 'column', background: C.bg, fontFamily: "'Fredoka', system-ui, sans-serif" }}>
      <style>{`
        .sk-topbar { flex-shrink: 0; display: flex; align-items: center; justify-content: space-between; padding: clamp(4px, .8vh, 10px) 8px; background: rgba(255,255,255,.5); border-bottom: 1px solid rgba(255,150,0,.15); }
        .sk-topbar-left { display: flex; align-items: center; gap: 4px; }
        .sk-topbar-right { display: flex; align-items: center; gap: clamp(6px, 1vw, 14px); }
      ...
      {/* Top bar: BackButton + progress + score */}
