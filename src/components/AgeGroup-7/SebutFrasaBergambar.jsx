...
// reads it aloud. STT is forgiving like Baca Ayat Kuat — passes when ≥60% of
// the keyword "slots" come through. Each slot has equivalent forms so we accept
// "baca"/"membaca", "kak"/"kakak", etc. Pairs with Baca Ayat Kuat (long ayat).

// ── Phases ────────────────────────────────────────────────────────────────────
const PHASE_SPEAKING  = 'speaking';   // TTS asking "Ceritakan gambar ini"
...
// ── Picture bank ────────────────────────────────────────────────────────────────
// phrase   = text shown under the picture + spoken by TTS as the model.
...
const PICTURES = [
  { id: 'p1',  emoji: '🐱🥛', phrase: 'Kucing minum susu',
    keywords: [['kucing','cat'], ['minum','drink'], ['susu','milk']] },
  { id: 'p2',  emoji: '🐦☁️', phrase: 'Burung terbang',
    keywords: [['burung','bird'], ['terbang','fly','flying']] },
  { id: 'p3',  emoji: '🐠💧', phrase: 'Ikan berenang',
    keywords: [['ikan','fish'], ['berenang','renang','swim','swimming']] },
  { id: 'p4',  emoji: '👦⚽', phrase: 'Budak menendang bola',
    keywords: [['budak','anak','boy','kid'], ['menendang','tendang','kick','kicking'], ['bola','ball']] },
  { id: 'p5',  emoji: '👧📚', phrase: 'Kakak membaca buku',
    keywords: [['kakak','kak','girl','sister'], ['membaca','baca','read','reading'], ['buku','book']] },
  { id: 'p6',  emoji: '🐰💨', phrase: 'Arnab melompat',
    keywords: [['arnab','rabbit','bunny'], ['melompat','lompat','jump','jumping','hop']] },
  { id: 'p7',  emoji: '👨🚗', phrase: 'Ayah memandu kereta',
    keywords: [['ayah','bapa','father','dad'], ['memandu','pandu','drive','driving'], ['kereta','car']] },
  { id: 'p8',  emoji: '👩‍🍳🍳', phrase: 'Ibu memasak',
    keywords: [['ibu','mak','emak','mother','mom'], ['memasak','masak','cook','cooking']] },
  { id: 'p9',  emoji: '👶😢', phrase: 'Adik menangis',
    keywords: [['adik','bayi','baby'], ['menangis','nangis','cry','crying']] },
  { id: 'p10', emoji: '👦💨', phrase: 'Abang berlari',
    keywords: [['abang','brother'], ['berlari','lari','run','running']] },
];
...
export default function SebutFrasaBergambar({ onBack, language = 'bm' }) {
  ...
  // Model the phrase aloud when entering SPEAKING phase — child listens then reads
  useEffect(() => {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: C.bg, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
          <div>
            <h1 style={{ color: C.primary, fontSize: '1.4rem', fontWeight: 900, marginBottom: '0.1rem' }}>
              🖼️ {language === 'bm' ? 'Baca Frasa Bergambar' : 'Read the Picture Phrase'}
            </h1>
            <p style={{ color: '#888', fontSize: '0.82rem' }}>
              {language === 'bm' ? 'Dengar, kemudian baca frasa di bawah gambar!' : 'Listen, then read the phrase under the picture!'}
            </p>
          </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0.5rem 1rem', maxWidth: '600px', width: '100%', alignSelf: 'center', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', justifyContent: 'center' }}>
        ...
        {/* Picture + phrase card */}
        <div style={{ background: cardBg, border: `3px solid ${cardBorder}`, borderRadius: '24px', padding: '1.5rem 1.5rem', textAlign: 'center', width: '100%', transition: 'background 0.3s, border-color 0.3s', boxShadow: '0 4px 16px rgba(0,0,0,0.06)' }}>
          <p style={{ fontSize: '0.8rem', fontWeight: 700, color: '#AAA', marginBottom: '0.5rem', letterSpacing: '0.5px' }}>
            {language === 'bm' ? 'BACA FRASA' : 'READ THE PHRASE'}
          </p>
          {item && (
            <>
              <div style={{ fontSize: '3.8rem', lineHeight: 1, marginBottom: '0.5rem' }}>{item.emoji}</div>
              <div style={{ fontSize: '1.85rem', fontWeight: 900, lineHeight: 1.3, color: isCorrect ? C.correctDark : isWrong ? C.wrongDark : '#333', transition: 'color 0.3s' }}>
                {item.phrase}
              </div>
            </>
          )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem', minHeight: '80px', justifyContent: 'center' }}>
          ...
          {phase === PHASE_SPEAKING && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#999', fontWeight: 700, fontSize: '0.9rem' }}>
              ...
              <span>{language === 'bm' ? 'Dengar contoh...' : 'Listen to the example...'}</span>
            </div>
          )}
        ...
        {(phase === PHASE_READY || phase === PHASE_LISTENING || phase === PHASE_SPEAKING) && (
          <div style={{ display: 'flex', gap: '1rem' }}>
            <button onClick={handleRepeat}
              ...
              title={language === 'bm' ? 'Ulang frasa' : 'Repeat phrase'}>
