...
// KSSR BM Tahun 1 — Obj 14 (menyoal & menjawab soalan secara lisan).
...
// it tests recall, not reading. Pairs with the Kata Tanya reading game.
...
// ── Question bank ───────────────────────────────────────────────────────────────
// answer = model answer shown/spoken; accept = normalised answers STT may hear.
const QUESTIONS = [
...
export default function JawabSoalan({ onBack, language = 'bm' }) {
  ...
  const handleWrong = () => {
    ...
    if (over) {
      if (item) speak(item.answer); // model the answer before moving on
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
          <div>
            <h1 style={{ color: C.primary, fontSize: '1.4rem', fontWeight: 900, marginBottom: '0.1rem' }}>
              💬 {language === 'bm' ? 'Jawab Soalan' : 'Answer the Question'}
            </h1>
            <p style={{ color: '#888', fontSize: '0.82rem' }}>
              {language === 'bm' ? 'Dengar soalan dan jawab dengan suara!' : 'Listen and answer out loud!'}
            </p>
          </div>
        ...
        <div style={{ background: cardBg, border: `3px solid ${cardBorder}`, borderRadius: '24px', padding: '1.75rem 1.5rem', textAlign: 'center', width: '100%', transition: 'background 0.3s, border-color 0.3s', boxShadow: '0 4px 16px rgba(0,0,0,0.06)' }}>
          ...
          {item && (
            <>
              ...
              <div style={{ fontSize: '1.5rem', fontWeight: 800, lineHeight: 1.35, color: isCorrect ? C.correctDark : isWrong ? C.wrongDark : '#333', transition: 'color 0.3s' }}>
