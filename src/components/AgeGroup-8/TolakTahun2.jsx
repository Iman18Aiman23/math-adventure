...
function generateQuestion(mechanic) {
  if (mechanic === 'gambar') {
    ...
    return {
      type: 'gambar',
      question_bm: 'Berapakah baki?',
      question_eng: 'How many left?',
      a, b,
      options: options.sort(() => Math.random() - 0.5).map(String),
      answer: String(answer),
      explanation_bm: `${a} - ${b} = ${answer}`,
    };
  } else if (mechanic === 'simbolik') {
    const a = Math.floor(Math.random() * 700) + 200; // 200-899
    const b = Math.floor(Math.random() * (a - 100)) + 50;
    const answer = a - b;
    const options = [answer];
    while (options.length < 3) {
      const wrong = answer + (Math.floor(Math.random() * 100) - 50);
      if (wrong >= 0 && wrong !== answer && !options.includes(wrong)) options.push(wrong);
    }
    return {
      type: 'simbolik',
    ...
    const b = Math.floor(Math.random() * (a - 50)) + 30;
    const answer = a - b;
    const stories = [
      { bm: `Kedai ada ${a} biji epal. Dijual ${b} biji. Berapa baki epal?`, eng: `Shop has ${a} apples. Sold ${b}. How many left?` },
      { bm: `Sekolah ada ${a} pelajar. ${b} pelajar tidak hadir. Berapa pelajar hadir?`, eng: `School has ${a} students. ${b} absent. How many present?` },
      { bm: `Ali ada RM${a}. Dia belanja RM${b}. Berapa baki Ali?`, eng: `Ali has RM${a}. He spent RM${b}. How much left?` },
    ];
    const story = stories[Math.floor(Math.random() * stories.length)];
    const options = [answer];
    while (options.length < 3) {
      const wrong = answer + (Math.floor(Math.random() * 50) - 25);
      if (wrong >= 0 && wrong !== answer && !options.includes(wrong)) options.push(wrong);
    }
    return {
      type: 'cerita',
...
export default function TolakTahun2({ onBack, language = 'bm' }) {
  ...
  const questions = useMemo(() => {
    const qs = [];
    ['gambar', 'simbolik', 'cerita'].forEach(m => {
  ...
  const handleResetGame = useCallback(() => {
    ...
    setStats({ gambar: { correct: 0, total: 0 }, simbolik: { correct: 0, total: 0 }, cerita: { correct: 0, total: 0 } });
  }, []);
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#D0F0FF', overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0, padding: '3.5rem 1rem 0.75rem', maxWidth: '600px', width: '100%', alignSelf: 'center', boxSizing: 'border-box' }}>
        <div style={{ textAlign: 'center', marginBottom: '0.85rem' }}>
          <h1 style={{ color: '#1CB0F6', marginBottom: '0.25rem', fontSize: '1.6rem' }}>{language === 'bm' ? 'Tolak Tahun 2' : 'Subtraction Grade 2'}</h1>
          <p style={{ color: '#888', fontSize: '0.9rem' }}>{language === 'bm' ? 'Tolak dalam lingkungan 1000' : 'Subtraction within 1000'}</p>
        </div>
        ...
        <div style={{ background: '#FFF', borderRadius: '12px', border: '2px solid #1CB0F6', padding: '1.1rem 1.25rem', marginBottom: '1rem' }}>
          ...
          {question.type === 'gambar' && (
            <div style={{ background: '#E3F2FD', borderRadius: '8px', padding: '1.5rem', marginBottom: '1rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(10, 1fr)', gap: '0.3rem', maxWidth: '320px', margin: '0 auto' }}>
                {Array(question.a).fill(0).map((_, i) => (
                  <span key={i} style={{ fontSize: '1.3rem', opacity: i < question.b ? 0.3 : 1, textDecoration: i < question.b ? 'line-through' : 'none' }}>🍎</span>
                ))}
              </div>
              <div style={{ fontSize: '1.3rem', fontWeight: 700, color: '#1CB0F6', marginTop: '1rem', textAlign: 'center' }}>{question.a} - {question.b} = ?</div>
            </div>
          )}

          {question.type === 'simbolik' && (
            <div style={{ background: '#FFF9C4', borderLeft: '4px solid #FBC02D', padding: '1.5rem', marginBottom: '1rem', borderRadius: '6px', textAlign: 'center' }}>
              <p style={{ fontSize: '1.8rem', color: '#333', margin: '0', fontWeight: 700 }}>{question.question_bm}</p>
            </div>
          )}

          {(question.type === 'cerita' || question.type === 'gambar') && (
