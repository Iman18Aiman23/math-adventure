...
function generateQuestion(mechanic) {
  if (mechanic === 'gambar') {
    ...
    return {
      type: 'gambar',
      question_bm: 'Berapa jumlah semuanya?',
      question_eng: 'What is the total?',
      a, b,
    ...
    const answer = a + b;
    const stories = [
      { bm: `Ali ada ${a} biji guli. Adi memberinya ${b} biji guli lagi. Berapa guli Ali sekarang?`, eng: `Ali has ${a} marbles. Adi gave him ${b} more. How many marbles now?` },
      { bm: `Kedai jual ${a} biji buku pada hari Isnin dan ${b} biji buku pada hari Selasa. Berapa jumlah buku?`, eng: `Shop sold ${a} books Monday and ${b} books Tuesday. Total?` },
      { bm: `Sekolah ada ${a} pelajar lelaki dan ${b} pelajar perempuan. Berapa jumlah pelajar?`, eng: `School has ${a} boys and ${b} girls. Total students?` },
    ];
...
export default function TambahTahun2({ onBack, language = 'bm' }) {
    ...
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#D0F0FF', overflow: 'hidden' }}>
      ...
      <div style={{ flexShrink: 0, padding: '3.5rem 1rem 0.75rem', maxWidth: '600px', width: '100%', alignSelf: 'center', boxSizing: 'border-box' }}>
        <div style={{ textAlign: 'center', marginBottom: '0.85rem' }}>
          <h1 style={{ color: '#1CB0F6', marginBottom: '0.25rem', fontSize: '1.6rem' }}>{language === 'bm' ? 'Tambah Tahun 2' : 'Addition Grade 2'}</h1>
          <p style={{ color: '#888', fontSize: '0.9rem' }}>{language === 'bm' ? 'Tambah dalam lingkungan 1000' : 'Addition within 1000'}</p>
        </div>
          ...
          {question.type === 'gambar' && (
            <div style={{ background: '#E3F2FD', borderRadius: '8px', padding: '1.5rem', marginBottom: '1rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(10, 1fr)', gap: '0.3rem', maxWidth: '320px' }}>
                  {Array(question.a).fill(0).map((_, i) => (
                    <span key={i} style={{ fontSize: '1.3rem' }}>🟢</span>
                  ))}
                </div>
                <span style={{ fontSize: '2rem', fontWeight: 'bold', color: '#1CB0F6' }}>+</span>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(10, 1fr)', gap: '0.3rem', maxWidth: '320px' }}>
                  {Array(question.b).fill(0).map((_, i) => (
                    <span key={i} style={{ fontSize: '1.3rem' }}>🔵</span>
                  ))}
                </div>
                <div style={{ fontSize: '1.3rem', fontWeight: 700, color: '#1CB0F6' }}>{question.a} + {question.b} = ?</div>
              </div>
            </div>
          )}
