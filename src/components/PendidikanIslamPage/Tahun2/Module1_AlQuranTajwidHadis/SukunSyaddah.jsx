...
const TOPICS_BELAJAR = [
  {
    id: 'sukun',
    title: 'Sukun (Tanda Mati)',
    desc: 'Tanda bulat kecil di atas huruf menunjukkan huruf itu dimatikan.',
    symbol: 'بْ',
    example: ['بْت', 'تْب', 'كْت', 'مْت'],
  ...
  },
  {
    ...
    desc: 'Tanda seperti huruf "w" kecil di atas huruf menunjukkan huruf ditekan dua kali.',
    symbol: 'بّ',
    example: ['بَّ', 'تَّ', 'كَّ', 'مَّ'],
...
const quizQuestions = [
  {
    question: 'Apakah tanda ini: بْ ?',
    options: ['Sukun', 'Syaddah', 'Fathah', 'Kasrah'],
    answer: 'Sukun',
  },
  {
    question: 'Apakah tanda ini: بّ ?',
    options: ['Kasrah', 'Syaddah', 'Sukun', 'Dammah'],
    answer: 'Syaddah',
  },
  {
    question: 'Huruf bersukun dibaca secara...',
    options: ['Panjang', 'Terputus/mati', 'Ditekan', 'Dengung'],
    answer: 'Terputus/mati',
  },
  {
    question: 'Huruf bersyaddah dibaca dengan...',
    options: ['Mati', 'Panjang', 'Tekanan dua kali', 'Dengung'],
    answer: 'Tekanan dua kali',
  },
];
...
export default function SukunSyaddah({ onBack, language = 'bm' }) {
  ...
  return (
    <Tahun1LessonLayout
      onBack={onBack}
      language={language}
      breadcrumb="Al-Quran, Tajwid & Hadis › Topik 1.1"
    ...
    >
      <style>{`
        ${FONT_IMPORT}
        .ss-grid { display: grid; grid-template-columns: 1fr; gap: 1rem; padding: 0 1.25rem; }
        @media (min-width: 640px) { .ss-grid { grid-template-columns: repeat(2, 1fr); gap: 1.1rem; } }
      `}</style>

      {tab === 'belajar' ? (
        <div className="ss-grid">
          {TOPICS_BELAJAR.map((t) => (
            <div key={t.id} style={{
        ...
        <div style={{ padding: '1rem 1.25rem', textAlign: 'center' }}>
          <div style={{
            ...
            <h3 style={{
              ...
              {language === 'bm' ? 'Kuiz Sukun & Syaddah' : 'Sukun & Syaddah Quiz'}
            </h3>
            <p style={{
              fontFamily: "'Fredoka', sans-serif",
              fontWeight: 600,
...
function ResultScreen({ score, totalRounds, onRetry, onBack, language }) {
  ...
  const msg = pct >= 80
    ? (language === 'bm' ? 'Hebat! Kamu faham Sukun & Syaddah!' : 'Great! You understand Sukun & Syaddah!')
    : pct >= 50
