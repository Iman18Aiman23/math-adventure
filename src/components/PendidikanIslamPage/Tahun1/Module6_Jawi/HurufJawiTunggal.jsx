...
export default function HurufJawiTunggal({ onBack, onNavigate, language = 'bm' }) {
  ...
  return (
    <Tahun1LessonLayout
      ...
      title={language === 'bm' ? 'Huruf Jawi Tunggal' : 'Single Jawi Letters'}
      accentColor="#0891B2"
      tab={tab}
    ...
    >
      ...
      {tab === 'belajar' ? (
        ...
        <QuizScreen key={quizKey} language={language} questions={QUESTIONS} totalRounds={TOTAL_ROUNDS} accentColor="#0891B2" onDone={(s) => { setFinalScore(s); setQuizDone(true); }} emoji="✍️" />
      )}
    </Tahun1LessonLayout>
  );
}
